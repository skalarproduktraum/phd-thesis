...
* __JavaVM/Fiji integration__ — Fiji[@Schindelin:2012ir] is a free and open-source image analysis software package running on the JavaVM, which is used by thousands of biologists world-wide. To enable easy adoption and present the user with a familiar ecosystem, the software has to integrate with this particular ecosystem easily. For that, the software has either to be written to target the Java Runtime Environment, or be able to be called from that.
